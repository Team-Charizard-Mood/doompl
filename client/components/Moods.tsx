...
export default function Moods({username}): JSX.Element {
  ...
  return (
    <Box sx={{ margin: "auto", width: "50vw", textAlign: "center" }}>
      ...
      <Button onClick={() => handleClick('chill')}>Chill</Button>
      <Button onClick={() => handleClick('sleep')}>Sleep</Button>
      <Button onClick={() => handleClick('workout')}>Workout</Button>
      <Button onClick={() => handleClick('angry')}>Angry</Button>
      <Button onClick={() => handleClick('drive')}>On a Drive</Button>
      <Button onClick={() => handleClick('study')}>Study</Button>
      <Button onClick={() => handleClick('party')}>Party</Button>
      <Button onClick={() => handleClick('surprise')}>Surprise</Button>
      <Typography><Link to={playlistUrl} target="_blank">{playlistUrl}</Link></Typography>
    </Box>
  );
}
